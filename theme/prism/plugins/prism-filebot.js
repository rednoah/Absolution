(function() {
	...
	Prism.languages.bash.executable = {
		pattern: /(^|[\s;|&]|[<>]\()(?:filebot(?:[.]sh)?|(?:fn|dev)[:][.a-z]+|true|false|\/(?:\w+\/)*bin\/[\w.-]+|TheMovieDB::TV|TheTVDB|AniDB|TheMovieDB|OMDb|AcoustID|ID3|xattr|file|exif|interactive|MOVE|COPY|SYMLINK|HARDLINK|KEEPLINK|DUPLICATE|CLONE|TEST|Airdate|DVD|Absolute|Digital|Story|Production|Date|SKIP|OVERRIDE|AUTO|INDEX|FAIL|artwork|cover|nfo|url|metadata|import|srt|subtitles|finder|tags|date|chmod|touch|thumbnail|reveal|prune|clean|refresh|OFF|SEVERE|WARNING|INFO|CONFIG|FINE|FINER|FINEST|ALL)(?=$|[)\s;|&])/i,
		lookbehind: true,
		alias: 'function'
	};
	...
	Prism.languages.groovy.binding = {
		...
		pattern: /\b(?:n|y|s|e|sxe|s00e00|t|d|airdate|startdate|absolute|ny|es|sy|sc|di|dc|age|special|episode|series|primaryTitle|alias|movie|id|tmdbid|tvdbid|imdbid|pi|pc|lang|subt|plex|kodi|emby|jellyfin|az|object|type|anime|regular|music|medium|album|artist|albumArtist|actors|director|collection|ci|cy|decade|genre|genres|language|languages|country|runtime|certification|rating|votes|vcf|vc|ac|cf|vf|hpi|aco|acf|af|channels|resolution|width|height|bitdepth|hdr|dovi|bitrate|vbr|abr|kbps|mbps|fps|khz|ar|ws|hd|dt|vs|source|edition|tags|s3d|group|original|historic|info|omdb|localize|order|db|fn|ext|f|folder|drive|files|relativeFile|mediaFile|mediaTitle|audioLanguages|textLanguages|duration|seconds|minutes|hours|bytes|megabytes|gigabytes|ct|crc32|media|video|audio|text|image|exif|camera|location|today|home|output|defines|label|self|model|episodelist|AnimeList|XEM|i|from|to|source|target|metadata|args|model|action|log|__\w+)(?:[.]\w+)*\b(?![({])/i,
		lookbehind: true,
		greedy: true,
		alias: 'variable',
		inside: {
	...
	Prism.languages.groovy.global = {
		pattern: /\b(?:null|none|any|allOf|concat|quote|toJson|include|text|csv|lines|xml|json|html|XML)\b/,
		alias: 'function'
	};
